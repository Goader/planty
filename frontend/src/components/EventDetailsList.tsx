...
function EventListItem(props: EventListItemProps) {
    return <ListGroup.Item className={'d-flex justify-content-between'}
        ...
        <p className={'m-0'}>{props.event.plant.name}</p>
    ...
    return (<>
        ...
        {events.water.length === 0 ? <p>No plants to water</p> :
            <ListGroup className={'mb-3'}>
                {events.water.map(event => <EventListItem event={event} key={event.plant.id}/>)}
            </ListGroup>
        }
        {/*<h5>Insolate</h5>*/}
        ...
        {events.fertilize.length === 0 ? <p>No plants to fertilize</p> :
            <ListGroup>
                {events.fertilize.map(event => <EventListItem event={event} key={event.plant.id}/>)}
            </ListGroup>
        }
    </>);
}
