...
function PlantDetailsView() {
  ...
  const [events, setEvents] = useState<CustomEventFormInfo[]>([]);

  const handleCreatedEvent = (event: CustomEventFormInfo) => {
      ...
      {plant !== null && plant !== undefined && (
        <Card className={"form-card p-5 mx-auto"}>
          ...
          <div className={"center-header"}>
            <h5>Picture:</h5>
            <Card.Img
              src={plant.photoUrl ? plant.photoUrl : "/default_plant.png"}
            />
          </div>
          ...
          <Button type={"submit"} className={"mt-3"}>
            Choose new care sheet
          </Button>

          <div
            className={
              "info-card mb-3 d-flex justify-content-between px-2 py-1"
            }
          >
          ...
          <div className={"center-header"}>
            <p>
              Nothing to change? <Link to={"/plants"}>back to garden</Link>
            </p>
          </div>
